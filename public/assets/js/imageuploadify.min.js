...
const retrieveFiles=(files)=>{for(
    let index=0;index<files.length;++index){
    ...
    const readingFile = (id, file) => {
        ...
        let container = $(`<div class='imageuploadify-container'><button type='button'class='btn btn-danger p-0 glyphicon glyphicon-remove'>&#x2715</button><div class='imageuploadify-details'><span>${file.name}</span><span>${file.type}</span><span>${file.size}</span></div></div>`);
    ...
    if(file.type.search(/audio/)!=-1){type="<i class='fa fa-file-audio-o'></i>";}
